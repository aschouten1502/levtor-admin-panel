...
import type { QATestRun, QATestQuestion, QACategory, CATEGORY_INFO } from '@/lib/admin/qa-types';
...
export default function TestResultsPage({ params }: PageProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(`/api/admin/test/${tenantId}/${runId}?include_questions=true`);
    ...
    const interval = setInterval(async () => {
      if (testRun && ['pending', 'generating', 'running', 'evaluating'].includes(testRun.status)) {
        try {
          const res = await fetch(`/api/admin/test/${tenantId}/${runId}/progress`);
          if (res.ok) {
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            {testRun.status === 'completed' && (
              <>
                <a
                  href={`/api/admin/test/${tenantId}/${runId}/pdf?format=pdf`}
                  download
                ...
                <a
                  href={`/api/admin/test/${tenantId}/${runId}/pdf?format=csv`}
                  download
